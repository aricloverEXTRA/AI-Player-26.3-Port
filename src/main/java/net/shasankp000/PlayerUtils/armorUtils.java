    ...
    public static void autoEquipArmor(ServerPlayerEntity bot) {
        ...
        for (EquipmentSlot slot : new EquipmentSlot[]{EquipmentSlot.HEAD, EquipmentSlot.CHEST, EquipmentSlot.LEGS, EquipmentSlot.FEET}) {
            ...
            if (!bestArmor.isEmpty() && (equippedArmor.isEmpty() || isBetterArmor(bestArmor, equippedArmor))) {
                ...
                if (armorSlotId != -1) {
                    bot.getInventory().setStack(PlayerInventory.MAIN_SIZE + armorSlotId, bot.getEquippedStack(slot).copy());
                }
            }
        }

        if (!equipmentUpdates.isEmpty()) {
            bot.getWorld().getPlayers().forEach(player ->
        ...
        for (EquipmentSlot slot : new EquipmentSlot[]{EquipmentSlot.HEAD, EquipmentSlot.CHEST, EquipmentSlot.LEGS, EquipmentSlot.FEET}) {
            ...
            if (!equippedArmor.isEmpty()) {
                ...
                if (inventory.insertStack(equippedArmor.copy())) {
                    ...
                    int armorSlotId = getArmorSlotId(slot);
                    if (armorSlotId != -1) {
                        bot.getInventory().setStack(PlayerInventory.MAIN_SIZE + armorSlotId, ItemStack.EMPTY);
                    }
